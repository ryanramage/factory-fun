...
exports.create = function () {
  ...
  factory.startProcessing = _startProcessing.bind(null, factory, g)
  return factory
}
...
function _addInput (factory, graph, amount, product, processingStepLabel, inputDetails, addedTimestamp, optionalEffortEstimate) {
  let node = graph.node(processingStepLabel)
  if (!node) throw Error(`Could not find node with processingStepLabel ${processingStepLabel}`)
...
function _startProcessing (factory, graph, processingStepLabel, startProcessingRequest, done) {
  done(null, '123')
  return factory
}

// function _sortInputs (factory, graph, processingStepLabel, sortFunction, done) {
//   let node = graph.node(processingStepLabel)
//   if (!node) throw Error(`Could not find node with processingStepLabel ${processingStepLabel}`)
//
//   let results = Joi.validate(sortFunction, sortFunctionSchema)
//   if (results.error) throw results.error
//
//   sortFunction(node.inputs, (err, sortedInputs) => {
